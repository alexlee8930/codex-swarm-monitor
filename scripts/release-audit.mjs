...
assert.match(workflow, /find release-artifacts -maxdepth 3 -type f -print/);
...
assert.match(uiCss, /--canvas: #f6f8fa/);
assert.match(uiCss, /max-width: 1920px/);
assert.match(uiCss, /\.canvas-proof-strip/);
assert.match(uiCss, /grid-template-columns: repeat\(5, minmax\(0, 1fr\)\)/);
assert.match(uiCss, /--notebook: #ffffff/);
assert.match(uiCss, /\.copy-icon::before/);
assert.match(uiCss, /\.sr-only/);
assert.match(uiCss, /\.repo-nav[\s\S]*var\(--wash\)/);
...
assert.match(readiness, /find dist -type f/);
...
assert.match(releaseRunbook, /gh release create v0\.1\.0 \$\(find dist -type f/);
