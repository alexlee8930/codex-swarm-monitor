...
assert.match(desktopBuilder, /WKWebView/, "desktop app must render the monitor inside a native WebKit app window");
assert.match(desktopBuilder, /"--workspace", FileManager\.default\.homeDirectoryForCurrentUser\.path, "--port", "0"/, "desktop app must start the monitor without opening the default browser or auto-installing hooks into home");
assert.doesNotMatch(desktopBuilder, /--workspace "\$HOME" --open/, "desktop app must not be a shell launcher that opens the default browser");
assert.match(desktopBuilder, /nativeWebView: true/, "desktop manifest must identify native WebView app behavior");
assert.match(desktopBuilder, /shellOpensBrowser: false/, "desktop manifest must prove the app is not a browser-opening shell wrapper");
...
assert.match(read("scripts/desktop-app-smoke.mjs"), /nativeWebView/);
assert.match(read("scripts/desktop-app-smoke.mjs"), /not a shell browser launcher/);
...
assert.match(remoteDesktopSmoke, /nativeWebView/);
assert.match(remoteDesktopSmoke, /not a shell browser launcher/);
