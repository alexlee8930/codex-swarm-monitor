...
# Project-Specific Overlay: Codex Swarm Monitor

This repo builds a local observability dashboard for Codex + OMX `/goal` and subagent work.

## Project Entry Points

- Backend: `apps/backend/src/index.mjs`
- Event store/state reducer: `apps/backend/src/store.mjs`
- Browser UI: `apps/ui/index.html`, `apps/ui/app.js`, `apps/ui/styles.css`
- Codex hook collector: `scripts/codex-swarm-hook.mjs`
- MCP subagent tool: `tools/agent-spawner/server.mjs`
- Product spec: `prd.md`
- Execution spec: `RALPH.md`
...
The backend uses Node 24's experimental `node:sqlite`, so SQLite warnings during tests are expected.
