...
macOS app wrapper archives are also built for:
...
Build identity and release posture are exposed through `codex-swarm-monitor --version`, `codex-swarm-monitor --version --json`, `GET /version`, `npm run release:readiness`, and `GET /release/readiness`. The Operations panel uses this metadata to show that the end-user prerequisite is Codex only, the primary end-user path is the macOS app or standalone bundle, mock data is disabled, realtime transport is SSE, and any remaining public-release blockers have concrete remediation steps. It also renders a release checklist with the exact local verification, tag, app artifact, standalone artifact, optional plugin package, and GitHub release commands required to move from source checkout to public assets. Standalone bundles also include `manifest.json` and `app/build-info.json` next to the bundled runtime.
